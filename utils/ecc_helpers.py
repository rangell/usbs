...
def warm_start_add_constraint(
    ...
    if old_sdp_state.X is not None:
        ...
        point_embeds = point_embeds.at[ecc_points].set(point_embeds.at[ecc_points] + 0.5 * avg_embed[None, :])
        #point_embeds = point_embeds.at[nbr_ecc_points].set(point_embeds[nbr_ecc_points] + (0.1 * avg_embed[None, :]))
    ...
    #y = y + ((1 / (1.0 * rho)) * SCALE_X * (jnp.clip(b - z, a_max=0.0) + (b - z) * (1 - b_ineq_mask)))
