...
class EccClusterer(object):
    ...
    def __init__(self,
        ...
        self.ineq_sdp_state = initialize_state(C=-C, sketch_dim=self.hparams.sketch_dim)
        self.eq_sdp_state = initialize_slack_state(C=-C, sketch_dim=self.hparams.sketch_dim)

        # call usbs on these back-to-back
        eq_start_time = time.time()
        _ = self._call_sdp_solver(self.eq_sdp_state, "usbs/cold")
        eq_end_time = time.time()

        ineq_start_time = time.time()
        _ = self._call_sdp_solver(self.ineq_sdp_state, "usbs/cold")
        ineq_end_time = time.time()

        print("eq time: ", eq_end_time - eq_start_time)
        print("ineq time: ", ineq_end_time - ineq_start_time)
